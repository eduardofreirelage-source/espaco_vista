...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // NOVO: Seletores para as novas seções
    ...
    async function fetchData() {
        try {
            const [servicesRes, tablesRes, pricesRes, quotesRes, paymentsRes, menusRes, menuItemsRes] = await Promise.all([
                ...
                supabase.from('menu_items').select('*, services(name)') // Pega o nome do serviço junto
            ]);
    ...
    // ... (funções de renderização existentes: renderQuotesTable, renderEventsTable, etc.) ...
    ...
    function renderPaymentMethods() {
        ...
        paymentMethods.forEach(method => {
            ...
            row.innerHTML = `
                <td><input type="text" class="editable-input" data-field="name" value="${method.name}"></td>
                <td class="actions">
                    <button class="btn-remove" data-action="delete-payment-method" data-id="${method.id}" title="Excluir">&times;</button>
                </td>
            `;
        });
    }
    ...
    function renderMenus() {
        if (!menusTbody || !selectMenuToEdit) return;
        menusTbody.innerHTML = '';
        selectMenuToEdit.innerHTML = '<option value="">-- Selecione --</option>';

        menus.forEach(menu => {
            const row = menusTbody.insertRow();
            row.dataset.id = menu.id;
            row.innerHTML = `
                <td><input type="text" class="editable-input" data-field="name" value="${menu.name}"></td>
                <td><input type="text" class="editable-input" data-field="description" value="${menu.description || ''}"></td>
                <td class="actions">
                    <button class="btn-remove" data-action="delete-menu" data-id="${menu.id}" title="Excluir">&times;</button>
                </td>
            `;
            const option = new Option(menu.name, menu.id);
            selectMenuToEdit.add(option);
        });
    }

    function populateGastronomyItemsSelect() {
        if (!gastronomyItemSelect) return;
        gastronomyItemSelect.innerHTML = '';
        ...
        gastronomyServices.forEach(service => {
            ...
            gastronomyItemSelect.add(option);
        });
    }

    function renderMenuItems(menuId) {
        if (!menuItemsTbody) return;
        menuItemsTbody.innerHTML = '';
        const items = menuItems.filter(item => item.menu_id == menuId);
        items.forEach(item => {
            const row = menuItemsTbody.insertRow();
            row.dataset.id = item.id;
            row.innerHTML = `
                <td>${item.services.name}</td>
                <td class="actions">
                    <button class="btn-remove" data-action="delete-menu-item" data-id="${item.id}" title="Remover item">&times;</button>
                </td>
            `;
        });
    }
    ...
    function addEventListeners() {
        ...
        // NOVO: Listeners para as novas seções
        ...
        document.getElementById('add-item-to-menu-form')?.addEventListener('submit', addItemToMenu);
        
        paymentMethodsTbody?.addEventListener('change', (e) => {
            if (e.target.classList.contains('editable-input')) {
                const id = e.target.closest('tr').dataset.id;
                updatePaymentMethod(id, e.target.dataset.field, e.target.value);
            }
        });

        menusTbody?.addEventListener('change', (e) => {
             if (e.target.classList.contains('editable-input')) {
                const id = e.target.closest('tr').dataset.id;
                updateMenu(id, e.target.dataset.field, e.target.value);
            }
        });

        selectMenuToEdit?.addEventListener('change', (e) => {
            if (e.target.value) {
        ...
        document.body.addEventListener('click', e => {
            ...
            // ... (ações existentes) ...
            if (action === 'delete-payment-method') deletePaymentMethod(id);
            if (action === 'delete-menu') deleteMenu(id);
            if (action === 'delete-menu-item') deleteMenuItem(id);
        });
    }
    ...
    async function addPaymentMethod(e) {
        e.preventDefault();
        const input = document.getElementById('paymentMethodName');
        const name = input.value.trim();
        if (!name) return;
        const { error } = await supabase.from('payment_methods').insert([{ name }]);
        if (error) { showNotification(`Erro: ${error.message}`, true); }
        else { showNotification('Forma de pagamento adicionada.'); input.value = ''; await fetchData(); }
    }

    async function updatePaymentMethod(id, field, value) {
        const { error } = await supabase.from('payment_methods').update({ [field]: value }).eq('id', id);
        if (error) { showNotification(`Erro ao atualizar.`, true); await fetchData(); }
    }
    ...
    async function deletePaymentMethod(id) {
        ...
        if (error) { showNotification(`Erro: ${error.message}`, true); }
        else { showNotification('Forma de pagamento excluída.'); await fetchData(); }
    }
    ...
    async function addMenu(e) {
        ...
        if (!name) return;
        const { error } = await supabase.from('menus').insert([{ name, description }]);
        if (error) { showNotification(`Erro: ${error.message}`, true); }
        else { showNotification('Cardápio adicionado.'); nameInput.value = ''; descInput.value = ''; await fetchData(); }
    }

    async function updateMenu(id, field, value) {
        const { error } = await supabase.from('menus').update({ [field]: value }).eq('id', id);
        if (error) { showNotification(`Erro ao atualizar.`, true); await fetchData(); }
    }
    ...
    async function deleteMenu(id) {
        ...
        if (error) { showNotification(`Erro: ${error.message}`, true); }
        else { showNotification('Cardápio excluído.'); await fetchData(); }
    }
    
    async function addItemToMenu(e) {
        e.preventDefault();
        const menu_id = selectMenuToEdit.value;
        const service_id = gastronomyItemSelect.value;
        if (!menu_id || !service_id) return;
        const { error } = await supabase.from('menu_items').insert([{ menu_id, service_id }]);
        if (error) { showNotification(`Erro: ${error.message}. O item já pode estar no cardápio.`, true); }
        else { showNotification('Item adicionado ao cardápio.'); await fetchData(); renderMenuItems(menu_id); }
    }
    ...
    async function deleteMenuItem(id) {
        ...
        if (error) { showNotification(`Erro: ${error.message}`, true); }
        else { showNotification('Item removido.'); await fetchData(); renderMenuItems(menu_id); }
    }


    // --- Funções existentes (sem alterações significativas) ---
    // (Cole aqui o restante do seu arquivo auth.js, desde a função formatCurrency até o final)
    // ...
