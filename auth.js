...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function renderGastronomyItems() {
        ...
        gastronomyServices.forEach(service => {
            ...
            row.innerHTML = `<td><input type="text" class="service-detail-input" data-field="name" value="${service.name}"></td><td>${createUnitSelect(service.unit)}</td><td class="actions"><button class="btn-remove" data-action="delete-service" data-id="${service.id}" title="Excluir Item">&times;</button></td>`;
        });
    }
    ...
    function renderAvailableGastronomyItems() {
        ...
        const availableItems = services.filter(s => s.category === 'Gastronomia' && !itemsInMenuIds.includes(s.id)).filter(s => s.name.toLowerCase().includes(searchQuery));
    ...
    async function addMenu(e) { e.preventDefault(); const nameInput = document.getElementById('menuName'); const name = nameInput.value.trim(); if (!name) return; const { data, error } = await supabase.from('menus').insert([{ name }]).select().single(); if (error) { showNotification(`Erro: ${error.message}`, true); } else { showNotification('Cardápio adicionado.'); nameInput.value = ''; await fetchData(); selectedMenuId = data.id; renderMenusList(); renderMenuDetails(); } }
    async function addItemToMenu(serviceId) { if (!selectedMenuId || !serviceId) return; const { error } = await supabase.from('menu_items').insert([{ menu_id: selectedMenuId, service_id: serviceId }]); if (error) { showNotification(`Erro: ${error.message}. O item já pode estar no cardápio.`, true); } else { await fetchData(); renderMenuDetails(); } }
    ...
    async function deleteMenu(id) { if (!confirm('Tem certeza? Isso excluirá o cardápio e todos os seus itens.')) return; const { error } = await supabase.from('menus').delete().eq('id', id); if (error) { showNotification(`Erro: ${error.message}`, true); } else { showNotification('Cardápio excluído.'); if (selectedMenuId === id) selectedMenuId = null; await fetchData(); renderMenuDetails(); } }
    async function deleteMenuItem(menuItemId) { if (!confirm('Remover este item?')) return; const { error } = await supabase.from('menu_items').delete().eq('id', menuItemId); if (error) { showNotification(`Erro: ${error.message}`, true); } else { await fetchData(); renderMenuDetails(); } }

    async function updateQuoteStatus(id, status) { const { error } = await supabase.from('quotes').update({ status }).eq('id', id); if (error) { showNotification(`Erro ao atualizar status: ${error.message}`, true); } else { showNotification('Status atualizado!'); await fetchData(); } }
