...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Função para aguardar bibliotecas externas carregarem
    function waitForGlobals(globals, callback) {
        const check = () => {
            if (globals.every(g => window[g] !== undefined)) {
                console.log("DEBUG: Bibliotecas globais carregadas. Executando callback.");
                callback();
            } else {
                console.log(`DEBUG: Aguardando as bibliotecas globais... Faltando: ${globals.filter(g => window[g] === undefined).join(', ')}`);
    ...
    function renderSalesFunnel() {
        // CORREÇÃO: Aguarda as bibliotecas estarem prontas antes de executar o código do gráfico
        waitForGlobals(['Chart', 'ChartFunnel', 'ChartDataLabels'], () => {
            try {
                console.log("DEBUG: 1. renderSalesFunnel foi chamada (após aguardar bibliotecas).");
                ...
                // O registro agora é feito aqui, com a garantia que os objetos existem
                Chart.register(window.ChartFunnel.FunnelController, ChartDataLabels);
